...
class SkillsTest(unittest.TestCase):
    ...
    def test_default_skills_dir_is_backend_skills(self):
        self.assertEqual(
            skills._skills_dir(),
            Path(__file__).resolve().parents[1] / "skills",
        )

    def test_relative_configured_skills_dir_resolves_from_backend(self):
        with patch.dict(os.environ, {"AGENT_SKILLS_DIR": "custom-skills"}):
            self.assertEqual(
                skills._skills_dir(),
                Path(__file__).resolve().parents[1] / "custom-skills",
            )

    def test_scan_skills_reads_backend_skills(self):
