...
- **故障恢复**：529 错误自动重试，并支持 fallback 模型降级。

### SSH 远程执行

- 系统 SSH 客户端调用，支持 `host` / `user` / `port` / `key_file` / `timeout`。
- 缺省参数自动回退到 `.agent_config.json` 中保存的 SSH 配置。
- 使用 `BatchMode=yes` 和 `StrictHostKeyChecking=accept-new` 确保非交互运行。
...
- 流式消息渲染，支持 Markdown、GFM、KaTeX 数学公式和代码语法高亮。
- 工具调用参数表格展示，大内容可折叠/展开。
- 支持消息编辑、重新提交、分支切换。
- 支持人机审批面板（Agent Inbox）。
...
- Stream throttle（50ms / 约 20fps），避免 SSE 高频更新阻塞主线程。
- `AssistantMessage` 使用 `React.memo` 和自定义 comparator，已完成消息跳过 re-render。
- ToolResult 折叠态使用纯字符串预览，展开态才执行 JSON parse/stringify。
- 移除 framer-motion `height:auto` 动画，减少布局重排。
...
| Skill | 说明 |
| --- | --- |
| youtube-downloader | 基于 yt-dlp 的视频下载，支持多分辨率、格式和仅音频模式。 |
| pdf | PDF 读取、生成、审阅，基于 reportlab、pdfplumber 和 pypdf。 |
...
- `task`（子 Agent）工具已实现但当前禁用，同步子 Agent 与 interrupt 流程调试困难。
- todo 和任务状态为内存存储，服务重启后会丢失；持久化任务保留在 `.tasks/` 目录。
