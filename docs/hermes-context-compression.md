...
| 近期保护 | token 预算加消息数下限 | 固定消息数 |
| 工具输出预裁剪 | 专用规则、去重、参数裁剪、图片清理 | 尚未实现完整预裁剪 |
...
1. 增加旧工具结果的一行摘要、重复结果去重和合法 JSON 参数裁剪。
2. 将固定 20 条尾部保护升级为 token 预算，并始终保留最新用户和助手消息。
3. 取消可能直接丢失中间信息的轻量 snip，或让 snip 只裁剪工具结果。
4. 增加显式滚动摘要，将旧摘要作为独立输入交给摘要模型更新。
5. 增加敏感信息过滤、失败中止和冷却。
6. 最后再考虑压缩锁、provider 角色适配和本地 fallback 等复杂边界。

第一阶段不需要改变 Qingzhou 的 LangGraph state 结构，适合作为低风险改进。
